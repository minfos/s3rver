...
var FileStore = function (rootDirectory,fs) {
  ...
  var getObjects = function (bucket, options, done) {
    ...
    var nextMarker = (filteredKeys.length > options.maxKeys) ? filteredKeys[options.maxKeys -1].replace(bucketPath + '/', '') : null;
  ...
  var deleteObject = function (bucket, key, done) {
    ...
    async.map([path.join(keyPath, METADATA_FILE),
      ...
      fs.rmdir(keyPath, function () {
        var parentPath = path.join(keyPath, "..");
        var bucketPath = path.resolve(path.join(rootDirectory, bucket.name));
        while (parentPath !== bucketPath && fs.existsSync(parentPath) && fs.readdirSync(parentPath).length === 0) {
