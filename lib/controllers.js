  ...
  return {
    ...
    getBucket: function (req, res) {
      ...
      if (req.query["list-type"] == 2) {
        options = {
          startAfter: req.query["start-after"] || null,
          marker: req.query["start-after"] || null,
          prefix: req.query.prefix || null,
          maxKeys: parseInt(req.query['max-keys']) || 1000,
          delimiter: req.query.delimiter || null,
          fetchOwner: !!req.query["fetch-owner"],
          v2: true,
        };
        if (req.query["continuation-token"]) {
          options.continuationToken = req.query["continuation-token"];
          options.marker = Buffer.from(options.continuationToken, "base64").toString();
        }
      } else {
        options = {
      ...
      if (indexDocument) {
        ...
        logger.info('Fetched bucket "%s" with options %s', req.bucket.name, JSON.stringify(options));
